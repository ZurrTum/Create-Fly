...
public class CreateCodecs {
    ...
    public static final Codec<BlockEntityType<?>> BLOCK_ENTITY_TYPE_CODEC = Registries.BLOCK_ENTITY_TYPE.getCodec();
    ...
    public static final Codec<Box> BOX_CODEC = Codec.DOUBLE.listOf()
        .xmap(
            data -> new Box(data.get(0), data.get(1), data.get(2), data.get(3), data.get(4), data.get(5)),
    ...
    public static final Codec<NbtList> NBT_LIST_CODEC = Codec.PASSTHROUGH.comapFlatMap(
        dynamic -> {
            ...
            return nbtElement instanceof NbtList nbtList ? DataResult.success(nbtList == dynamic.getValue() ? nbtList.copy() : nbtList) : DataResult.error(
    ...
    public static final Codec<Couple<Vec3d>> COUPLE_VEC3D_CODEC = Couple.codec(Vec3d.CODEC);
    public static final Codec<Couple<Integer>> COUPLE_INT_CODEC = Couple.codec(Codec.INT);
    public static final Codec<Set<UUID>> UUID_SET_CODEC = Uuids.INT_STREAM_CODEC.listOf().xmap(ImmutableSet::copyOf, ImmutableList::copyOf);
    public static final Codec<List<NbtCompound>> NBT_COMPOUND_LIST_CODEC = NbtCompound.CODEC.listOf();
