...
public class ContinuousOBBCollider extends OBBCollider {
    ...
    public static ContinuousSeparationManifold separateBBs(Vec3d cA, Vec3d cB, Vec3d eA, Vec3d eB, Matrix3d m, Vec3d motion) {
        ...
        if (
            ...
            !(separate(mf, uA0, diff.x, eA.x, a00 * eB.x + a01 * eB.y + a02 * eB.z, motion.x, true) || separate(
                mf,
                uA1,
                diff.y,
                eA.y,
    ...
    static boolean separate(
        ...
        if (dot != 0 && discreteCollision) {
            ...
            if (!cross.equals(Vec3d.ZERO)) {
                ...
                stepSeparationVec = sepVec.subtract(axisPlane.multiply(sepVec.dotProduct(stepPlane) / axisPlane.dotProduct(stepPlane)));
        ...
        if (isBestSeperation) {
            ...
            mf.collisionPosition = normalizedAxis.multiply(signum(TL) * (axisOfObjA ? -rB : -rB) - signum(seperation) * .125f);
        }
