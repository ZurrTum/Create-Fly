...
public class CTSpriteShiftEntry extends SpriteShiftEntry {
    ...
    public float getTargetU(float localU, int index) {
        ...
        return getTarget().getFrameU((getUnInterpolatedU(getOriginal(), localU) + uOffset) / ((float) type.getSheetSize()));
    }

    public float getTargetV(float localV, int index) {
        float vOffset = (index / type.getSheetSize());
        return getTarget().getFrameV((getUnInterpolatedV(getOriginal(), localV) + vOffset) / ((float) type.getSheetSize()));
    }
