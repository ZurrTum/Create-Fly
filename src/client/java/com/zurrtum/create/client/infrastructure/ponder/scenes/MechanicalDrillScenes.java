...
public class MechanicalDrillScenes {
    ...
    public static void breaker(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i == 1) {
                scene.overlay().showText(80).attachKeyFrame().placeNearTarget().pointAt(util.vector().topOf(breakingPos))
        ...
        ElementLink<EntityElement> plankEntity = scene.world()
            .createItemEntity(util.vector().centerOf(breakingPos), util.vector().of(0, .1f, 0), new ItemStack(Items.OAK_PLANKS));
        scene.idle(20);
        scene.idle(15);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            scene.world().incrementBlockBreakingProgress(breakingPos);
            if (i == 2) {
                scene.overlay().showText(80).attachKeyFrame().placeNearTarget().pointAt(util.vector().topOf(breakingPos.east()))
    ...
    public static void contraption(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showText(60).attachKeyFrame().placeNearTarget().pointAt(util.vector().topOf(util.grid().at(3, 2, 3)))
        ...
        scene.overlay().showText(60).attachKeyFrame().placeNearTarget().pointAt(util.vector().blockSurface(util.grid().at(4, 3, 2), Direction.WEST))
        ...
        scene.world().setKineticSpeed(drills, 0);
        scene.idle(10);
        scene.overlay().showControls(util.vector().topOf(2, 3, 2), Pointing.DOWN, 60).withItem(new ItemStack(Blocks.OAK_PLANKS));
        scene.idle(20);
    }
