...
public class ExtendoGripModel implements ItemModel, SpecialModelRenderer<ExtendoGripModel.RenderData> {
    ...
    public void update(
        ...
        if (stack == null) {
            ...
            resolver.update(data.item, stack, displayContext, world, user, seed);
            if (user == null) {
                user = MinecraftClient.getInstance().player;
            }
            data.flip = rightHand ^ user.getMainArm() == Arm.LEFT ? 1 : -1;
        }
    ...
    public void render(
        ...
        if (data.self) {
            ...
            render(data, displayContext, matrices, vertexConsumers, light, overlay, glint);
            matrices.pop();
        ...
        ItemRenderer.renderItem(displayContext, matrices, vertexConsumers, light, overlay, grip.tints, grip.getQuads(), grip.renderLayer, grip.glint);
        ...
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, thinShort);
        matrices.translate(0, 0.34375f, 0);
        matrices.multiply(RotationAxis.NEGATIVE_X.rotationDegrees(data.oppositeAngle));
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, wideLong);
        ...
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, wideShort);
        matrices.translate(0, 0.34375f, 0);
        matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(data.oppositeAngle));
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, thinLong);
        matrices.translate(0, 0.6875f, 0);
        matrices.multiply(RotationAxis.NEGATIVE_X.rotationDegrees(data.oppositeAngle));
        matrices.translate(0, 0.03125f, 0);
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, wideShort);
        ...
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, data.hand);
        matrices.pop();
        ...
        matrices.translate(-0.5f, -0.5625f, -0.5f);
        renderQuads(displayContext, matrices, vertexConsumers, light, overlay, cog);
        matrices.pop();
    }
