...
public class FanScenes {
    ...
    public static void direction(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showText(80).text("Encased Fans use Rotational Force to create an Air Current").attachKeyFrame().placeNearTarget()
        ...
        scene.overlay().showText(60).text("Strength and Direction of Flow depends on the Rotational Input").placeNearTarget()
            .pointAt(util.vector().topOf(fanPos));
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60).colored(PonderPalette.BLACK).attachKeyFrame()
        ...
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60).colored(PonderPalette.MEDIUM).attachKeyFrame()
        ...
        scene.overlay().showText(100).colored(PonderPalette.RED).pointAt(util.vector().topOf(blockPos.east())).placeNearTarget().attachKeyFrame()
