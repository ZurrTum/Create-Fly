...
public class East extends Face {
    private static int computeFlag(
        ...
        boolean b0 = y0 >= EPS_MAX, b1 = z0 <= EPS_MIN;
        boolean b2 = y1 >= EPS_MAX, b3 = z1 <= EPS_MIN;
        boolean b4 = y2 >= EPS_MAX, b5 = z2 <= EPS_MIN;
        boolean b6 = y3 >= EPS_MAX, b7 = z3 <= EPS_MIN;
        boolean b8 = x >= EPS_MAX;
    ...
    public static void prepareQuad(
        ...
        switch (computeFlag(y0, z0, y1, z1, y2, z2, y3, z3, x0)) {
            case CUBIC_FLAG_0 -> fullFlipFace0(
                cache.computeEastBlock(level, state, pos),
                cache.computeEast(level, state, pos),
                x0,
                output
            );
            case LIGHT_FACE_FLAG | CUBIC_FLAG_0 -> fullFace0(cache.computeEastBlock(level, state, pos), output);
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_1 -> fullFace1(cache.computeEastBlock(level, state, pos), output);
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_2 -> fullFace2(cache.computeEastBlock(level, state, pos), output);
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_3 -> fullFace3(cache.computeEastBlock(level, state, pos), output);
            ...
            case LIGHT_FACE_FLAG | PARTIAL_FLAG ->
                partialFace0(cache.computeEastBlock(level, state, pos), y0, z0, y1, z1, y2, z2, y3, z3, output);
        }
    }
    ...
    public static void prepareShadeQuad(
        ...
        switch (computeFlag(y0, z0, y1, z1, y2, z2, y3, z3, x0)) {
            case CUBIC_FLAG_0 -> fullFlipFace0(
                cache.computeEastBlockShade(level, state, pos),
                cache.computeEastShade(level, state, pos),
                x0,
                output
            );
            case LIGHT_FACE_FLAG | CUBIC_FLAG_0 -> fullFace0(cache.computeEastBlockShade(level, state, pos), output);
            case CUBIC_FLAG_1 -> fullFlipFace1(
                cache.computeEastBlockShade(level, state, pos),
                cache.computeEastShade(level, state, pos),
                x0,
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_1 -> fullFace1(cache.computeEastBlockShade(level, state, pos), output);
            case CUBIC_FLAG_2 -> fullFlipFace2(
                cache.computeEastBlockShade(level, state, pos),
                cache.computeEastShade(level, state, pos),
                x0,
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_2 -> fullFace2(cache.computeEastBlockShade(level, state, pos), output);
            case CUBIC_FLAG_3 -> fullFlipFace3(
                cache.computeEastBlockShade(level, state, pos),
                cache.computeEastShade(level, state, pos),
                x0,
            ...
            case LIGHT_FACE_FLAG | CUBIC_FLAG_3 -> fullFace3(cache.computeEastBlockShade(level, state, pos), output);
            case PARTIAL_FLAG -> partialFlipFace0(
                cache.computeEastBlockShade(level, state, pos),
                cache.computeEastShade(level, state, pos),
                x0,
            ...
            case LIGHT_FACE_FLAG | PARTIAL_FLAG ->
                partialFace0(cache.computeEastBlockShade(level, state, pos), y0, z0, y1, z1, y2, z2, y3, z3, output);
        }
    }
    ...
    private static void irregularFace(
        ...
        float w = 1 - clamp(x), u0 = clamp(y), v0 = clamp(z);
        float u1 = 1 - u0, v1 = 1 - v0;
        if (w < 0.00001f) {
            write(i, cache.computeEastBlock(level, state, pos), u0, v0, u1, v1, output);
        } else if (w > 0.99999f) {
            write(i, cache.computeEast(level, state, pos), u0, v0, u1, v1, output);
        } else {
    ...
    private static void irregularShadeFace(
        ...
        float u1 = 1 - u0, v1 = 1 - v0;
        if (w < 0.00001f) {
            write(i, cache.computeEastBlockShade(level, state, pos), u0, v0, u1, v1, output);
        } else if (w > 0.99999f) {
            write(i, cache.computeEastShade(level, state, pos), u0, v0, u1, v1, output);
        } else {
