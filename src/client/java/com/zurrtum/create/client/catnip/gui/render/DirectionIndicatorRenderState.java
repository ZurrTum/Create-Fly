...
public record DirectionIndicatorRenderState(Matrix3x2f pose, float r, float g, float b, ScreenRect bounds) implements SimpleGuiElementRenderState {
    ...
    public void setupVertices(VertexConsumer vertexConsumer) {
        vertexConsumer.vertex(pose, 0, 0).color(r, g, b, 1);
        vertexConsumer.vertex(pose, 5, -5).color(r, g, b, 0.6f);
        vertexConsumer.vertex(pose, 3, -4.5f).color(r, g, b, 0.7f);
        vertexConsumer.vertex(pose, 0, -4.2f).color(r, g, b, 0.7f);
        vertexConsumer.vertex(pose, -3, -4.5f).color(r, g, b, 0.7f);
        vertexConsumer.vertex(pose, -5, -5).color(r, g, b, 0.6f);
    }
